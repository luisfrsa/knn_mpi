...
int main(int argc, char *argv[]){	
	/*MPI*/
	int id_mpi, size,i,val,max,min, local_val;
	MPI_Init(&argc,&argv);
	MPI_Status st;
	...
	char *line = NULL,
	     *split,
		 ...
		 //dir_test[100]="bases/test_59.data",
		 //dir_treino[100]="bases/train_59.data";
	...
	for(int i=id_mpi;i<size_teste;i+=size){
		menor_distancia = FLT_MAX;
		memset(&melhor_musica, 0, sizeof(melhor_musica));
		calcula_menor_distancia(casos_teste[i],casos_treino,qdeElem,size_treino);
	...
	if(id_mpi==0){
		for(i=1;i<size;i++){//para todos os processos da aplicação
			MPI_Recv(&retorno_acertos,1,MPI_INT,i,TAG,MPI_COMM_WORLD,&st);//recebe valor
			MPI_Recv(&retorno_erros,1,MPI_INT,i,TAG,MPI_COMM_WORLD,&st);//recebe valor
			acertos+=retorno_acertos;
			erros+=retorno_erros;
		}
		printf("\nQuantidade de acertos: %d\nQuantidade de erros: %d\nTotal: %d\n",acertos,erros,(erros+acertos));
		printf("\n");
	}else{
		MPI_Send(&acertos,1,MPI_INT,0,TAG,MPI_COMM_WORLD);
		MPI_Send(&erros,1,MPI_INT,0,TAG,MPI_COMM_WORLD);
	}
